...
class MarketData:
    def __init__(self, symbol: str, api_key: str = None, api_secret: str = None):
        ...
        # Initialize Stock Historical Data Client with timeout
        self.api = StockHistoricalDataClient(
            self.api_key, 
            self.api_secret,
    ...
    async def get_market_data(self):
        ...
        try:
            ...
            bars = await asyncio.wait_for(
                self.api.get_stock_bars(params),
                timeout=30
            )
            df = bars.df
            ...
            # Get latest quote with timeout
            quote_params = StockLatestQuoteRequest(symbol_or_symbols=self.symbol)
            quote = await asyncio.wait_for(
                self.api.get_stock_latest_quote(quote_params),
                timeout=30
            )
            ...
            logger.error(f"Timeout while fetching market data for {self.symbol}")
